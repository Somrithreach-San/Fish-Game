...
namespace Rhinotap.Toolkit
{
    public class EventManager : MonoBehaviour
    {
        ...
        public static void StopListening<ActionType>(string eventName, Action<ActionType> listener)
        {
            if (_instance == null)
            {
                // Silent return or warning if strict debugging is needed.
                // For cleanup purposes, we usually just want to return if the system is already gone.
                return;
            }

            if ( _instance.events.KeyExists<Action<ActionType>>(eventName))
            {
                Dictionary<string, Action<ActionType>> currentDictionary = _instance.events.GetDictionary<Action<ActionType>>();
                if( currentDictionary[eventName] != null)
                {
                    currentDictionary[eventName] -= listener;
                    _instance.events.UpdateDictionary<Action<ActionType>>(currentDictionary);
        ...
        public static void StopListening(string eventName, Action listener)
        {
            ...
            if (_instance.voidEvents.ContainsKey(eventName))
            {
                _instance.voidEvents[eventName] -= listener;
            }
        }
